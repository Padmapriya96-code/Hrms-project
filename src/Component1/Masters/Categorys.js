...
const Categorys = () => {
  ...
  const fetchCategories = useCallback(async () => {
    if (companyId && branchId) {
      try {
        const catData = await postRequest(ServerConfig.url, REPORTS, {
          query: `
            SELECT pn_CategoryID, v_CategoryName, status, pn_CompanyID, BranchID
  ...
  }, [companyId, branchId, fetchCategories]);
