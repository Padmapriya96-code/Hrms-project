...
const CategoryMaster1 = () => {
  ...
  const fetchCategoryData = async () => {
    if (pnCompanyId) {
      const categoryData = await postRequest(ServerConfig.url, REPORTS, {
        query: `SELECT [pn_CompanyID], [BranchID], [pn_CategoryID], [v_CategoryName], [status] 
                FROM [dbo].[paym_Category] WHERE pn_CompanyID = '${pnCompanyId}'`,
      });
      const formattedData = categoryData.data.map((item) => ({
        pnCompanyId: item.pn_CompanyID,
        BranchID: item.BranchID,
        pn_CategoryID: item.pn_CategoryID,
  ...
  const isRowModified = useCallback(
    (rowData) => {
      ...
      const originalRow = originalData.find(
        (item) => item.pn_CategoryID === rowData.pn_CategoryID
      );
      if (!originalRow) return false;
      return (
        String(originalRow.BranchID) !== String(rowData.BranchID) ||
        String(originalRow.v_CategoryName) !== String(rowData.v_CategoryName) ||
    ...
    (params) => {
      ...
      if (!rowData.isNew) {
        ...
        setModifiedRows((prev) => ({
          ...prev,
          [rowData.pn_CategoryID]: modified,
        }));
      }
  ...
  const handleAddRow = () => {
    ...
    if (lastRow && (!lastRow.BranchID || !lastRow.v_CategoryName || !lastRow.status)) {
      toast.dismiss();
  ...
  const handleSaveAll = async () => {
    try {
      ...
      for (const row of newRows) {
        if (!row.BranchID || !row.v_CategoryName || !row.status) {
      ...
      const queries = newRows.map((row) => {
        ...
        return `INSERT INTO paym_Category (pn_CompanyID, BranchID, v_CategoryName, status) 
                VALUES ('${row.pnCompanyId}', '${row.BranchID}', '${row.v_CategoryName}', '${statusChar}')`;
      });
  ...
  const handleUpdateRow = async (rowData) => {
    try {
      ...
      const query = `UPDATE paym_Category 
                     SET BranchID = '${rowData.BranchID}', 
                         v_CategoryName = '${rowData.v_CategoryName}', 
                         status = '${statusChar}' 
                     WHERE pn_CategoryID = '${rowData.pn_CategoryID}'`;
      await postRequest(ServerConfig.url, SAVE, { query });
      setModifiedRows((prev) => {
        const newModified = { ...prev };
        delete newModified[rowData.pn_CategoryID];
        return newModified;
      });
  ...
  const executeDelete = async (rowData) => {
    try {
      const query = `DELETE FROM [dbo].[paym_Category] WHERE [pn_CategoryID] = '${rowData.pn_CategoryID}'`;
      await postRequest(ServerConfig.url, SAVE, { query });
        ...
        cellRenderer: (params) => (
          <div>
            <IconButton
              ...
              color="success"
              disabled={!modifiedRows[params.data.pn_CategoryID]}
            >
              <DoneOutlineIcon />
            </IconButton>
...
export default CategoryMaster1;
