...
const JobStatusFormMaster1 = () => {
  ...
  const fetchJobStatusData = async () => {
    if (pnCompanyId) {
      const jobStatusData = await postRequest(ServerConfig.url, REPORTS, {
        query: `SELECT [pn_CompanyID], [BranchID], [pn_JobStatusID], [v_JobStatusName], [status] 
                FROM [dbo].[paym_JobStatus] WHERE pn_CompanyID = '${pnCompanyId}'`,
      });
      const formattedData = jobStatusData.data.map((item) => ({
        pnCompanyId: item.pn_CompanyID,
        BranchID: item.BranchID,
        pn_JobStatusID: item.pn_JobStatusID,
  ...
  const isRowModified = useCallback(
    (rowData) => {
      ...
      const originalRow = originalData.find(
        (item) => item.pn_JobStatusID === rowData.pn_JobStatusID
      );
      if (!originalRow) return false;
      return (
        String(originalRow.BranchID) !== String(rowData.BranchID) ||
        String(originalRow.v_JobStatusName) !== String(rowData.v_JobStatusName) ||
    ...
    (params) => {
      ...
      if (!rowData.isNew) {
        ...
        setModifiedRows((prev) => ({
          ...prev,
          [rowData.pn_JobStatusID]: modified,
        }));
      }
  ...
  const handleAddRow = () => {
    ...
    if (lastRow && (!lastRow.BranchID || !lastRow.v_JobStatusName || !lastRow.status)) {
      toast.dismiss();
  ...
  const handleSaveAll = async () => {
    try {
      ...
      for (const row of newRows) {
        if (!row.BranchID || !row.v_JobStatusName || !row.status) {
      ...
      const queries = newRows.map((row) => {
        ...
        return `INSERT INTO paym_JobStatus (pn_CompanyID, BranchID, v_JobStatusName, status) 
                VALUES ('${row.pnCompanyId}', '${row.BranchID}', '${row.v_JobStatusName}', '${statusChar}')`;
      });
  ...
  const handleUpdateRow = async (rowData) => {
    try {
      ...
      const query = `UPDATE paym_JobStatus 
                     SET BranchID = '${rowData.BranchID}', 
                         v_JobStatusName = '${rowData.v_JobStatusName}', 
                         status = '${statusChar}' 
                     WHERE pn_JobStatusID = '${rowData.pn_JobStatusID}'`;
      await postRequest(ServerConfig.url, SAVE, { query });
      setModifiedRows((prev) => {
        const newModified = { ...prev };
        delete newModified[rowData.pn_JobStatusID];
        return newModified;
      });
  ...
  const executeDelete = async (rowData) => {
    try {
      const query = `DELETE FROM [dbo].[paym_JobStatus] WHERE [pn_JobStatusID] = '${rowData.pn_JobStatusID}'`;
      await postRequest(ServerConfig.url, SAVE, { query });
    ...
    () => [
      {
        ...
        valueGetter: (params) => {
          const branchItem = branch.find((b) => b.pn_BranchID === params.data.BranchID);
          return branchItem?.BranchName || "";
        },
        valueSetter: (params) => {
          const selectedBranch = branch.find((b) => b.BranchName === params.newValue);
          if (selectedBranch) {
        ...
        cellRenderer: (params) => (
          <div>
            <IconButton
              ...
              color="success"
              disabled={!modifiedRows[params.data.pn_JobStatusID]}
            >
              <DoneOutlineIcon />
            </IconButton>
...
export default JobStatusFormMaster1;
