...
const GradeMaster1 = () => {
  ...
  const fetchGradeData = async () => {
    if (pnCompanyId) {
      const gradeData = await postRequest(ServerConfig.url, REPORTS, {
        query: `SELECT [pn_CompanyID], [BranchID], [pn_GradeID], [v_GradeName], [status] 
                FROM [dbo].[paym_Grade] WHERE pn_CompanyID = '${pnCompanyId}'`,
      });
      const formattedData = gradeData.data.map((item) => ({
        pnCompanyId: item.pn_CompanyID,
        BranchID: item.BranchID,
        pn_GradeID: item.pn_GradeID,
        v_GradeName: item.v_GradeName,
  ...
  const isRowModified = useCallback(
    (rowData) => {
      ...
      const originalRow = originalData.find(
        (item) => item.pn_GradeID === rowData.pn_GradeID
      );
      if (!originalRow) return false;
      return (
        String(originalRow.BranchID) !== String(rowData.BranchID) ||
        String(originalRow.v_GradeName) !== String(rowData.v_GradeName) ||
  ...
  const handleAddRow = () => {
    ...
    if (lastRow && (!lastRow.BranchID || !lastRow.v_GradeName || !lastRow.status)) {
      toast.dismiss();
  ...
  const handleSaveAll = async () => {
    try {
      ...
      for (const row of newRows) {
        if (!row.BranchID || !row.v_GradeName || !row.status) {
      ...
      const queries = newRows.map((row) => {
        ...
        return `INSERT INTO paym_Grade (pn_CompanyID, BranchID, v_GradeName, status) 
                VALUES ('${row.pnCompanyId}', '${row.BranchID}', '${row.v_GradeName}', '${statusChar}')`;
      });
  ...
  const handleUpdateRow = async (rowData) => {
    try {
      ...
      const query = `UPDATE paym_Grade 
                     SET BranchID = '${rowData.BranchID}', 
                         v_GradeName = '${rowData.v_GradeName}', 
                         status = '${statusChar}' 
                     WHERE pn_GradeID = '${rowData.pn_GradeID}'`;
      await postRequest(ServerConfig.url, SAVE, { query });
      setModifiedRows((prev) => {
        const newModified = { ...prev };
        delete newModified[rowData.pn_GradeID];
        return newModified;
      });
  ...
  const executeDelete = async (rowData) => {
    try {
      const query = `DELETE FROM [dbo].[paym_Grade] WHERE [pn_GradeID] = '${rowData.pn_GradeID}'`;
      await postRequest(ServerConfig.url, SAVE, { query });
        ...
        cellRenderer: (params) => (
          <div>
            <IconButton
              ...
              color="success"
              disabled={!modifiedRows[params.data.pn_GradeID]}
            >
              <DoneOutlineIcon />
            </IconButton>
