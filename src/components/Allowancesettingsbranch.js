...
const Allowancesettingsbranch = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const queries = rowData
        .filter((row) => {
          // Ensure at least one preference is selected
          return Object.keys(deductionPreferences).some((deductionName) => {
            const columnName = deductionName.replace(/\s+/g, '_');
            return deductionPreferences[deductionName] && row[columnName] === "Yes";
          });
        })
        .map(async (row, index) => {
...
export default Allowancesettingsbranch;