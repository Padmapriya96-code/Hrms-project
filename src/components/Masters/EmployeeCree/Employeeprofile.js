...
export default function PaymEmployeeFormm() {
  ...
  //             REPORTS,
  //             {
  //               query: `SELECT * FROM [${databaseName}].[dbo].[paym_Company]WHERE pn_CompanyID = ${loggedBranchData.data[0].pn_CompanyID}`,
  //             }
  //           );
  ...
  useEffect(() => {
    async function getData() {
      try {
        ...
        // ✅ Prepare all queries for master data
        const queries = [
          `SELECT * FROM [${databaseName}].[dbo].[paym_Company]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Branch]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Division]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Department]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Designation]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Grade]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Shift]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Category]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_JobStatus]`,
          `SELECT * FROM [${databaseName}].[dbo].[paym_Level]`,
        ];

        // Compolsory need to work on these contollers
        ...
        // ] = await Promise.all(
        //   queries.map((query) =>
        //     postRequest(ServerConfig.url, REPORTS, { query })
        //   )
        // );

        // // ✅ Update all dropdown state values
        // setCompany(companies.data);
        // setBranch(branches.data);
        // setDivision(divisions.data);
        // setDepartment(departments.data);
        ...
        // ✅ Fetch logged branch & company for current user
        if (isloggedin) {
          const loggedBranchData = await postRequest(
            ServerConfig.url,
            REPORTS,
          ...
          if (loggedBranchData.data && loggedBranchData.data.length > 0) {
            ...
            if (loggedCompanyData.data && loggedCompanyData.data.length > 0) {
              setloggedCompany(loggedCompanyData.data);
              setPnCompanyId(loggedCompanyData.data[0].pn_CompanyID);
            }
          }
  ...
  useEffect(() => {
    async function fetchInitialData() {
      try {
        ...
        // ✅ Prepare queries for Company & Branch
        const companyQuery = `SELECT * FROM [${databaseName}].[dbo].[paym_Company];`;
        const branchQuery = `SELECT * FROM [${databaseName}].[dbo].[paym_Branch];`;

        // ✅ Fetch both simultaneously
        const [companyData, branchData] = await Promise.all([
          postRequest(ServerConfig.url, REPORTS, { query: companyQuery }),
          postRequest(ServerConfig.url, REPORTS, { query: branchQuery }),
        ]);
        ...
        console.log("✅ Company & Branch data loaded successfully.");
      } catch (error) {
  ...
  useEffect(() => {
      ...
      try {
        if (pnCompanyId && pnBranchId) {
          const response = await postRequest(ServerConfig.url, REPORTS, {
            query: `SELECT DISTINCT AssetType FROM [${databaseName}].[dbo].[Assets] 
    ...
    fetchAssetTypes();
  }, [pnCompanyId, pnBranchId]); // Add dependencies here
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Helper function to properly escape SQL values
    ...
    [pn_CompanyID], [pn_BranchID], [EmployeeCode], [Password],
    [Employee_First_Name], [Employee_Middle_Name], [Employee_Last_Name],
    [DateofBirth], [Gender], [status], [Employee_Full_Name], [Readerid],
    [OT_Eligible], [Pfno], [Esino], [OT_calc], [CTC], [basic_salary],
    [Bank_code], [Bank_Name], [Branch_Name], [Account_Type], [MICR_code],
    [IFSC_Code], [Other_Info], [Reporting_person], [ReportingID],
    [Reporting_email], [Pan_no], [salary_type], [TDS_Applicable], [Flag],
    [role], [accountNo], [Blood_Group], [Phone_No], [Alternate_Phone_No],
    [permanent_address], [Aadhar_Card], [Current_Address], [Father_Name],
    [Email], [Alternate_Email], [Grade], [Overall_Experience],
    [HighestQualification], [UniversityName], [YearOfPassing],
    [Certifications], [Skills], [UAN], [PaymentMode], [PassportNumber],
    [VisaDetails], [JoiningDate], [ExitReason], [PreviousCompany],
    [PreviousDesignation], [PreviousEmploymentDuration],
    [ReasonForLeaving], [PerformanceRating], [TrainingRecords],
    [DisciplinaryActions], [Awards], [VehicleDetails],
    [HealthInsuranceDetails], [NomineeDetails], [Asset_Name],
    [Asset_SerialNumber], [NomineePhoneno], [NomineeRelationship],
    [ExitDate], [AssetType]
   )
   VALUES (
    ${sqlValue(formData.pn_CompanyID)}, ${sqlValue(formData.pn_BranchID)},
    ${sqlValue(formData.employeeCode)}, ${sqlValue(formData.password)},
    ${sqlValue(formData.employeeFirstName)}, ${sqlValue(
          formData.employeeMiddleName
        )},
    ${sqlValue(formData.employeeLastName)}, ${sqlValue(formData.dateofBirth)},
    ${sqlValue(formData.gender)}, ${sqlValue(formData.status)},
    ${sqlValue(formData.employeeFullName)}, ${sqlValue(formData.readerid)},
    ${sqlValue(formData.otEligible)}, ${sqlValue(formData.pfno)},
    ${sqlValue(formData.esino)}, ${sqlValue(formData.otCalc)},
    ${sqlValue(formData.ctc)}, ${sqlValue(formData.basicSalary)},
    ${sqlValue(formData.bankCode)}, ${sqlValue(formData.bankName)},
    ${sqlValue(formData.branchName)}, ${sqlValue(formData.accountType)},
    ${sqlValue(formData.micrCode)}, ${sqlValue(formData.ifscCode)},
    ${sqlValue(formData.otherInfo)}, ${sqlValue(formData.reportingPerson)},
    ${sqlValue(formData.reportingId)}, ${sqlValue(formData.reportingEmail)},
    ${sqlValue(formData.panNo)}, ${sqlValue(formData.salaryType)},
    ${sqlValue(formData.tdsApplicable)}, ${sqlValue(formData.flag)},
    ${sqlValue(formData.role)}, ${sqlValue(formData.accountNo)},
    ${sqlValue(formData.bloodGroup)}, ${sqlValue(formData.phoneNo)},
    ${sqlValue(formData.alternatePhoneNo)}, ${sqlValue(
          formData.permanentAddress
        )},
    ${sqlValue(formData.aadharCard)}, ${sqlValue(formData.currentAddress)},
    ${sqlValue(formData.fatherName)}, ${sqlValue(formData.email)},
    ${sqlValue(formData.alternateEmail)}, ${sqlValue(formData.grade)},
    ${sqlValue(formData.overallExperience)}, ${sqlValue(
          formData.highestQualification
        )},
    ${sqlValue(formData.universityName)}, ${sqlValue(formData.yearOfPassing)},
    ${sqlValue(formData.certifications)}, ${sqlValue(formData.skills)},
    ${sqlValue(formData.uan)}, ${sqlValue(formData.paymentMode)},
    ${sqlValue(formData.passportNumber)}, ${sqlValue(formData.visaDetails)},
    ${sqlValue(formData.joiningDate)}, ${sqlValue(formData.exitReason)},
    ...
    ${sqlValue(formData.nomineeDetails)}, ${sqlValue(formData.assetName)},
    ${sqlValue(formData.assetSerialNumber)}, ${sqlValue(
          formData.NomineePhoneno
        )},
    ${sqlValue(formData.NomineeRelationship)}, ${sqlValue(formData.ExitDate)},
    ${sqlValue(formData.assetType)}
    )`.replace(/\n/g, " ");
      ...
      SELECT IDENT_CURRENT('[${databaseName}].[dbo].[paym_Employee]') AS employeeId;
    `;
      ...
      INSERT INTO [${databaseName}].[dbo].[paym_employee_profile1] (
        ...
        ${sqlValue(formData.pnCategoryId)}, ${sqlValue(
    ...
    `.replace(/\n/g, " ");
      ...
      // 2️⃣ INSERT EMPLOYEE LOGIN INTO HRMS_Master (static DB)
      const masterLoginQuery = `
      INSERT INTO [HRMS_Master].[dbo].[EmployeesLogin]
      ([EmployeeUserId], [Password], [DBname], [employeeFullName])
      VALUES 
      ('${formData.employeeCode}', '${formData.password}', '${databaseName}', '${formData.employeeFullName}');
      `;

      const masterRes = await postRequest(ServerConfig.url, REPORTS, {
        query: masterLoginQuery,
      });

      if (masterRes.status === 200) {
        console.log("Employee Login created in HRMS_Master");
      } else {
        console.warn("Failed to insert login into HRMS_Master");
      }
                        ...
                        <Grid item xs={12} sm={6}>
                          ...
                          >
                            ...
                            <Select
                              ...
                              disabled={isViewMode || !formData.assetType}
                            >
                              <MenuItem value="">Select Asset</MenuItem>
                              {assets
                                .filter(
                                  (asset) =>
                                    asset.AssetType === formData.assetType
                                )
                        ...
                        <Grid item xs={12} sm={6}>
                          ...
                          >
                            ...
                            {assets
                              .filter(
                                (asset) =>
                                  asset.Asset_name === formData.assetName
                              )
                              .filter(
                                (asset) =>
                  ...
                  {activeStep < steps.length - 1 ? (
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: "5px",
                        border: "none",
                        backgroundColor: "#0b87da", // Green for finish
                        color: "white",
                        cursor: "pointer",
                        fontWeight: "bold",
                      }}
