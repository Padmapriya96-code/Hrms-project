...
//           query: `SELECT * FROM paym_Department WHERE pn_BranchID = ${branchId};`,
//         });
...
//   useEffect(() => {
//     if (loggedBranch.length > 0) fetchDepartments();
//   }, [loggedBranch]);

//   const handleSearchChange = (event) => setSearchQuery(event.target.value);

//   const handleEditClick = (department) => {
//     navigate(`/DepartmentMaster`, { state: { departmentData: department } });
//   };
...
//           <Button variant="contained" size="small" color="red" onClick={async () => {
//             toast.dismiss();
//             await executeDelete(id);
//           }} style={{ minWidth: "80px" }}>
//             Delete
//           </Button>
//           <Button variant="outlined" size="small" onClick={() => toast.dismiss()} style={{ minWidth: "80px" }}>
//             Cancel
//           </Button>
//         </div>
//       </div>,
...
//       query: `DELETE FROM [dbo].[paym_Department] WHERE pn_DepartmentID = ${id};`,
//     });
...
//         autoClose: 1000,
//       });
//       await fetchDepartments(); // refresh grid
//     } else {
...
//     const name = department.v_DepartmentName ? department.v_DepartmentName.toLowerCase() : "";
...
//         <Box sx={{ display: "flex" }}>
//           <Sidenav />
//           <Grid item xs={12} sm={10} md={8} style={{ margin: '0 auto', display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center' }}>
...
//             <Card className={classes.card} style={{ marginTop: "0px" , borderRadius: "0 0 20px 20px",  // ✅ bottom-only radius
//     overflow: "hidden"   }}>
//               <CssBaseline />
...
//                           <TableRow key={department.id || index} style={{ backgroundColor: index % 2 === 0 ? '#cde3f2' : '#ffffff' , height: 30}}>
//                             <TableCell>{index + 1}</TableCell>
//                             <TableCell>{department.v_DepartmentName}</TableCell>
//                             <TableCell>{department.status === 'A' ? 'Active' : 'Inactive'}</TableCell>
//                             <TableCell className={classes.actionCell}>
//                               <IconButton aria-label="edit" style={{ color: '#007BFF' }} onClick={() => handleEditClick(department)}>
//                                 <EditIcon />
//                               </IconButton>
//                              <IconButton
//   aria-label="delete"
//   style={{ color: 'red' }}
//   onClick={() => handleDeleteClick(department.pn_DepartmentID)} // ✅ fix here
// >
//   <DeleteOutlineIcon />
// </IconButton>
...
function DepartmentHome() {
  ...
  const [loggedBranch, setLoggedBranch] = useState([]);
  const navigate = useNavigate();
  const canManageDept = Boolean(loggedBranch[0]?.can_manage_department);


  // ✅ Get DB name from session storage
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const refreshValue = sessionStorage.getItem("forceRefreshAccess");
      if (refreshValue) {
        fetchBranchData(); // re-fetch branch access
      }
  ...
  const fetchDepartments = async () => {
    if (loggedBranch.length > 0) {
      ...
      try {
        if (!databaseName) {
          console.warn("⚠️ Missing database name for department query");
          return;
        }
  ...
  // ✅ DELETE with dynamic DB
  const executeDelete = async (id) => {
    try {
      setIsLoading(true);

      if (!databaseName) {
        console.warn("⚠️ No database name for delete operation");
        return;
      }

      const deleteQuery = `
        DELETE FROM [${databaseName}].[dbo].[paym_Department] 
  ...
  const filteredDepartments = departments.filter((department, index) => {
    const name = department.v_DepartmentName
      ? department.v_DepartmentName.toLowerCase()
      : "";
    const status = department.status === "A" ? "active" : "inactive";
    const sNo = (index + 1).toString();
  ...
  <TableRow>
    ...
    {canManageDept && <TableCell>Actions</TableCell>}
  </TableRow>
</TableHead>
  ...
  {isLoading ? (
    <TableRow>
      <TableCell colSpan={canManageDept ? 4 : 3} align="center">
        <CircularProgress />
      </TableCell>
    </TableRow>
  ) : filteredDepartments.length > 0 ? (
    filteredDepartments.map((department, index) => (
      <TableRow
        key={department.pn_DepartmentID || index}
        style={{
        ...
        {canManageDept && (
          <TableCell className={classes.actionCell}>
            ...
            <IconButton
              ...
              style={{ color: "red" }}
              onClick={() =>
                handleDeleteClick(department.pn_DepartmentID)
              }
            >
              <DeleteOutlineIcon />
            </IconButton>
